...
test.describe('Quiz library — additional edge cases', () => {
  ...
  test('"Build your first quiz" CTA on the empty state navigates to the creator', async ({ page }) => {
    await page.goto('/');
  ...
  test('shows "1 question" (singular) for a single-question quiz', async ({ page }) => {
    ...
    await page.goto('/#/quizzes');
    await page.waitForLoadState('networkidle');
    await page.getByText(/1 question/i).waitFor({ state: 'visible', timeout: 15_000 });
    await expect(page.getByText(/1 question/i)).toBeVisible();
    await expect(page.getByText(/1 questions/i)).not.toBeVisible();
  });

  test('shows "N questions" (plural) for a multi-question quiz', async ({ page }) => {
    const multiQQuiz = {
      ...SAMPLE_QUIZ,
      id: 'quiz-multi',
      title: 'Multi-Q',
