...
import { seedQuiz, SAMPLE_QUIZ } from '../helpers';

test.describe('Home screen — edge cases', () => {
  // ── PIN input guard ────────────────────────────────────────────────────────

  test('does NOT navigate when the typed PIN is shorter than 4 characters', async ({ page }) => {
    await page.goto('/');
  ...
  test('PIN input converts lowercase to uppercase automatically', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
  ...
  test('shows "No quizzes saved" copy when no quizzes exist', async ({ page }) => {
    await page.goto('/');
  ...
  test('shows the saved quiz count in the "Host a quiz" section', async ({ page }) => {
    await seedQuiz(page);
    await page.reload();
    await page.waitForLoadState('networkidle');
  ...
  test('navigates to join screen when the PIN is exactly 4 characters', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
